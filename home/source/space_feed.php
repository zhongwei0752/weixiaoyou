...
$thisday=array();
$xml = new DOMDocument(); 
$xml->load("http://www.todayonhistory.com/rss.xml"); 
//$i='1';
foreach($xml->getElementsByTagName('item') as $list1) 
	{ 
		//$i++;
		$authors = $list1->getElementsByTagName( "title" );
...
foreach($guoneinewsxml->getElementsByTagName('item') as $guonei) 
	{ 
    	...
    	$guoneipubDate = $pubDate->item(0)->nodeValue;
    		...
    		$guoneinews1['title']=$guoneititle;
    		$guoneinews1['link']=$guoneilink;
    		$guoneinews1['description']=$guoneidescription;
    		$guoneinews1['pubDate']=$guoneipubDate;  
    		$guoneinews2[]=$guoneinews1;
		
	} 
	$guoneinews=array_slice($guoneinews2,0,5);
	//$newguoneinews=implode(",",$newguoneinews);
//国外新闻
$guowainews=array();
...
foreach($guowainewsxml->getElementsByTagName('item') as $guowai) 
	{ 
    	...
    	$guowaipubDate = $pubDate->item(0)->nodeValue;
    		...
    		$guowainews1['title']=$guowaititle;
    		$guowainews1['link']=$guowailink;
    		$guowainews1['description']=$guowaidescription;
    		$guowainews1['pubDate']=$guowaipubDate;  
    		$guowainews2[]=$guowainews1;	
		
	} 
			$guowainews=array_slice($guowainews2,0,5);	
	//娱乐电影新闻
$yulenews=array();
...
foreach($yulenewsxml->getElementsByTagName('item') as $yule) 
	{ 
    	...
    	$yulepubDate = $pubDate->item(0)->nodeValue;
    		...
    		$yulenews1['title']=$yuletitle;
    		$yulenews1['link']=$yulelink;
    		$yulenews1['description']=$yuledescription;
    		$yulenews1['pubDate']=$yulepubDate;  
    		$yulenews2[]=$yulenews1;	
		
	} 
			$yulenews=array_slice($yulenews2,0,5);
	//体育新闻
$tiyunews=array();
//$rssfeed = array();
//echo $rssfeed[0];
...
foreach($tiyunewsxml->getElementsByTagName('item') as $tiyu) 
	{ 
    	...
    	$tiyupubDate = $pubDate->item(0)->nodeValue;
    		...
    		$tiyunews1['title']=$tiyutitle;
    		$tiyunews1['link']=$tiyulink;
    		$tiyunews1['description']=$tiyudescription;
    		$tiyunews1['pubDate']=$tiyupubDate;  
    		$tiyunews[]= $tiyunews1;	
		
	} 
//}	
	//判断情况选择显示
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('appset')." where uid='$_SGLOBAL[supe_uid]'");
